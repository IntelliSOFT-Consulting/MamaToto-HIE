...
export const issueConsent = async (patientId: string, facilityId: string) => {
    try {
        ...
        if (consent.resourceType != "Consent") {
            consent = (await FhirApi(`/Consent/${patientData?.id}`, {
                method: 'PUT',
                data: {
