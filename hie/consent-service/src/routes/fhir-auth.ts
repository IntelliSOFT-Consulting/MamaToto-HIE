...
router.post('/fhir', async (req: Request, res: Response) => {
    ...
    if (!currentUser) {
        res.statusCode = 401;
        res.json(OperationOutcome("Invalid Bearer token provided"));
        return;
    }
...
router.get('/fhir/Patient/:id', async (req: Request, res: Response) => {
    try {
        ...
        let consentResources = (await FhirApi(`/Consent?patient=Patient/${id}`)).data;

        if (!consentResources || consentResources.total === 0) {
            return res.status(401).json(OperationOutcome("No consent found for the given patient ID"));
        }
        let isConsentValid = false;
        for (let entry of consentResources.entry) {
            if (entry.resource.organization[0].reference.includes(currentUser.family_name)) {
                isConsentValid = true;
                break;
            }
        }
...
router.get('/fhir/Patient', async (req: Request, res: Response) => {
    try {
        ...
        let patientResource = (await FhirApi(`/Patient${searchParams.toString() ? `?${searchParams.toString()}` : ''}`));
        return res.status(patientResource.statusCode).json(patientResource.data);
    } catch (error) {
        console.error(error);
        return res.status(500).json(OperationOutcome("Internal Server Error"));
